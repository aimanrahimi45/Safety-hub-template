...
const DOC_NAME = "Akta-Petroleum-langkah-langkah-keselamatan-1984.pdf";
const TEXTPATH = "/tmp/akta302_text.txt";
...
const SCHEDULE_RE = /^JADUAL\s*(PERTAMA|KEDUA|KETIGA|KEEMPAT|KELIMA)?\s*/;
...
function findContentStart(lines) {
  for (let i = 0; i < lines.length; i++) {
    if (/^MAKA INILAH/.test(lines[i].trim())) return i;
  }
  return 0;
}
...
async function run() {
  console.log("Parsing Akta 514 text...");
