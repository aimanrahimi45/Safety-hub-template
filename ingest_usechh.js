...
const DOC_NAME = "Peraturan-Keselamatan-dan-Kesihatan-Pekerjaan-Penggunaan-dan-Standard-Pendedahan-Bahan-Kimia-Berbahaya-Kepada-Kesihatan-2000.pdf";
const TEXTPATH = "/tmp/usechh_text.txt";
...
const SCHEDULE_RE = /^JADUAL\s*(PERTAMA|KEDUA|KETIGA|KEEMPAT|KELIMA|I{1,3})?\s*/;
...
function findContentStart(lines) {
  for (let i = 0; i < lines.length; i++) {
    if (/^1\.\s/.test(lines[i].trim())) return i;
  }
  return 0;
}
...
async function run() {
  console.log("Parsing USECHH Regulations text...");
  ...
  if (existingClauses && existingClauses.length > 0) {
    const ids = existingClauses.map(c => c.id);
    ...
    console.log(`Deleted ${ids.length} existing clauses + obligations.`);
  }
